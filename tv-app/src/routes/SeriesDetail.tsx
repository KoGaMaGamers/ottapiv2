 ...
 *   └────────────────────────────────────────────────────────┘
 *
 * Three D-pad zones: actions, seasons, episodes. Up at actions →
 * TopNav; Down from episodes is a no-op. Selecting a season chip
 * with ←/→ auto-loads its episodes — no Enter needed (matches
 * Netflix-style instant feedback).
 *
 * Real Play behavior wires in step 12. Today, Enter on an episode
 * card logs an info message.
 */
...
export default function SeriesDetail() {
  ...
  const [episodes] = createResource(
    ...
    (k) => (k ? getSeasonEpisodes(k.sid, k.sn) : Promise.resolve([])),
  );
  ...
  function moveHorizontal(delta: 1 | -1) {
    const cur = zone();
    if (cur === "actions") {
      setActionIdx((i) => Math.min(Math.max(i + delta, 0), ACTIONS.length - 1));
      return;
    }
    if (cur === "seasons") {
      const ss = seasons();
      if (ss.length === 0) return;
      const nextIdx = Math.min(Math.max(seasonIdx() + delta, 0), ss.length - 1);
    ...
    if (cur === "actions") {
      ...
      switch (a.key) {
        case "watchlist":
          ...
          console.info("[series-detail] watchlist toggle (step 15)");
          break;
        case "back":
          history.back();
          break;
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="flex gap-3 text-sm text-zinc-400 mb-4">
                  ...
                  <Show when={s.tmdb_vote_average != null}>
                    <span class="text-yellow-400">★ {s.tmdb_vote_average!.toFixed(1)}</span>
                  </Show>
                </div>
                ...
                <div class="flex gap-3">
                  <For each={ACTIONS}>
                    {(action, i) => {
                      const focused = () =>
                        isScopeOwner() && zone() === "actions" && actionIdx() === i();
                      return (
                        <button
                          onClick={() => {
                            setZone("actions");
                ...
                <div class="flex gap-2 flex-wrap">
                  <For each={seasons()}>
                    {(season, i) => {
                      ...
                      const isFocused = () =>
                        isScopeOwner() && zone() === "seasons" && seasonIdx() === i();
                      return (
                        <button
                          onClick={() => {
                            setZone("seasons");
